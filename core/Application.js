  ...
  constructor(
    ...
    Promise.all(
      ...
      routes => {
        ...
        this.router.use(urlencodedParser({ extended: true }))
        if (cookie)
        ...
        server.use(
          this.router
          // (request, response, next) => {
          //   console.log(request.accepts('json'), request.header('accept'))
        ...
        // server.app.set('view engine', 'backend');
        // server.app.engine(
        //   'backend',
        //   (filename, options, cb) => {
        //     console.log(filename, options, cb)
        //     cb(null, 'Hello world')
        //   }
        // )
      }
